...
public class Equipment {
	...
	public static enum Slot {
		HELMET(7, 0, 0), CAPE(10, 1, 1), NECK(13, 2, 2), WEAPON(16, 3, 3), BODY(
				19, 4, 4), SHIELD(22, 5, 5), LEGS(25, 7, 7), HANDS(28, 9, 9), FEET(
				31, 10, 10), RING(34, 12, -1), AMMO(39, 13, -1), AURA(48, 14,
				14), POCKET(70, 15, -1);
		...
		Slot(final int componentIndex, final int bankComponentIndex,
		 ...
		 * 
		 * @return the index of the slot for either the bank or the regular
		 *         Equipment tab
		 */
		public int getIndex() {
		 ...
		 * 
		 * @param bank
		 *            <tt>true</tt> to get component index of bank equipment
		 *            component otherwise <tt>false</tt> for regular index
	 ...
	 * 
	 * @param itemIds
	 *            the item ids to look for
	 * @return <tt>true</tt> if all of the provided item ids are visible items;
	 ...
	 * @return <tt>true</tt> if one of the provided items is visibly equipped;
	 *         otherwise <tt>false</tt>
	 ...
	 * @param itemIds
	 *            the item ids to look for
	 * @return <tt>true</tt> if all of the provided item ids are equipped items;
	 *         otherwise <tt>false</tt>
	 ...
	 * @param itemIds
	 *            the item ids to check for
	 * @return <tt>true</tt> if one of the provided items is an equipped item;
	 *         otherwise <tt>false</tt>
	 * @see #containsAll(int...)
	 */
	 ...
	 * 
	 * @param slot
	 *            the <code>Slot</code> of the item
	 * @return the item id at the specified <code>Slot</code>; otherwise
	 *         <code>-1</code> if not visible
	 */
	public static int getAppearanceId(final Slot slot) {
		if (slot.getAppearanceIndex() == -1) {
			return -1;
		}
		final int slotId = Players.getLocal().getAppearance()[slot
				.getAppearanceIndex()];
	 ...
	 * 
	 * @return an array instance of integers representing the local player's
	 *         visible equipment.
	 */
	...
	/**
	 * Gets all the equipped items. If the tab is not currently open, it will
		...
		if (widget != null) {
			...
			if (components.length > 0) {
				...
				for (int i = 0; i < NUM_SLOTS; i++) {
					items[i] = new Item(
							components[slots[i].getComponentIndex()]);
				}
				return items;
			}
		}
	...
	/**
	 * Gets the count of all equipped items matching with any of the provided
	 * ids ignoring stack sizes.
	 * 
	 * @param itemIds
	 *            the item ids to look for
	 * @return the count
	...
	public static Item getItem(final Slot slot) {
		...
		if (widget != null && widget.validate()) {
			try {
				final WidgetChild itemComp = (widget.getIndex() == WIDGET_BANK) ? widget
						.getChild(COMPONENT_EQUIP_INVENTORY).getChild(
								slot.getBankComponentIndex()) : widget
						.getChild(slot.getComponentIndex());
				if (itemComp != null) {
	...
	/**
	 * Gets the first item in the inventory with an id matching the argument(s)
	 * from the last known array of items
	 * 
	 * @param itemIds
	 *            the id(s) to search for
	 * @return an <code>Item</code> with an id matching the argument(s)
	...
	/**
	 * Gets the equipped item at the given index from the last known array of
	 * items.
	 * 
	 * @param slot
	 *            the <code>Slot</code> of the item
	...
	public static Item[] getItems() {
		...
		if (widget != null) {
			...
			final WidgetChild[] equip = (isBank) ? widget.getChild(
					COMPONENT_EQUIP_INVENTORY).getChildren() : widget
					.getChildren();
			if (equip.length > 0) {
				if (!isBank) {
					final Item[] items = new Item[NUM_SLOTS];
	 ...
	 * 
	 * @param itemIds
	 *            the ids of the item(s) that need(s) to be equipped
	 * @return <tt>true</tt> if an item was successfully equipped otherwise
	 *         <tt>false</tt>
	 */
	public static boolean equip(final int... itemIds) {
		...
		if (item != null) {
			...
			for (String action : item_child.getActions()) {
				...
				if (action.contains("Equip") || action.contains("Wear")
						|| action.contains("Wield")) {
	 ...
	 * 
	 * @param itemIds
	 *            the ids of the item(s) that need(s) to be unequipped
	 * @return <tt>true</tt> if an item was successfully unequipped otherwise
	 *         <tt>false</tt>
	 */
