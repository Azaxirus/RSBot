...
public class TilePath extends Path {
	...
	public Tile getNext() {
		...
		for (int i = tiles.length - 1; i >= 0; --i) {
			if (tiles[i].isOnMap()
					&& (tiles[i].canReach() || i != 0
							&& (dest.getX() != -1 ? Calculations.distance(dest,
									tiles[i - 1]) < 3 : Calculations
									.distanceTo(tiles[i - 1]) < 7)))
				return tiles[i];
		}
		return null;
	}
	...
	public TilePath randomize(final int maxX, final int maxY) {
		for (int i = 0; i < tiles.length; ++i) {
			tiles[i] = orig[i].derive(Random.nextInt(-maxX, maxX + 1),
					Random.nextInt(-maxY, maxY + 1));
		}
		return this;
	}
	...
	public boolean traverse(final EnumSet<TraversalOption> options) {
		...
		if (next.equals(getEnd())) {
			if (Calculations.distanceTo(next) <= 1
					|| end
					&& (!Players.getLocal().isMoving() || Calculations
							.distanceTo(next) < 3))
				return false;
			end = true;
		} else {
		...
		if (options != null && options.contains(TraversalOption.HANDLE_RUN)
				&& !Walking.isRunEnabled() && Walking.getEnergy() > 50) {
			Walking.setRun(true);
			Task.sleep(300);
		}
		if (options != null && options.contains(TraversalOption.SPACE_ACTIONS)) {
			final Tile dest = Walking.getDestination();
			if (dest.getX() != -1 && Players.getLocal().isMoving()
					&& Calculations.distanceTo(dest) > 5
					&& Calculations.distance(next, dest) < 7)
				return true;
		}
