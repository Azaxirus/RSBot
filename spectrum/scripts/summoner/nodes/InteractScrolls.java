...
public class InteractScrolls implements Strategy {
				...
				if (Widgets.get(1370).validate()) {
					if (Widgets.get(1370, 20) != null) {
						if (Inventory.contains(new int[] { Variables.pouchId })
								&& !Inventory
										.contains(new int[] { Variables.scrollId })) {
	...
	public boolean validate() {
		return Variables.obeliskArea.contains(Players.getLocal())
				&& !Variables.doneWithObelisk || !Variables.doneWithObelisk
				|| Variables.walkToObelisk;
	}
}